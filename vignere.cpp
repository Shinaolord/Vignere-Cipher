...
*/
    ...
    dekey[j]=div(26-(toupper(key[j])-65),26).rem +65;
      std::cout<< int (toupper(key[j)])<<" Is the value of key, " << int(dekey[j]-65)<<"\n"<<std::endl;  };

     /* WHY DOESN'T THE DECRYPT WORK WHEN plaintext.length() >51 ??????? TRY MOVING  DEKEY GEN OUT OF for LOOP?? */
